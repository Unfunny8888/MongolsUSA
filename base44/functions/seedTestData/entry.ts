...
Deno.serve(async (req) => {
  try {
    ...
    // Create groups first
    const groupNames = [
      { name: 'Ulaanbaatar Expats', description: 'Connect with international community in Mongolia', category: 'community' },
      { name: 'Tech & Innovation', description: 'Discuss latest tech trends and startups', category: 'professional' },
      { name: 'Hiking Enthusiasts', description: 'Explore beautiful trails together', category: 'social' },
      { name: 'Mongolian Food Lovers', description: 'Share recipes and restaurant recommendations', category: 'social' },
      { name: 'Business Networking', description: 'Meet local entrepreneurs and professionals', category: 'business' },
      { name: 'Art & Culture', description: 'Celebrate Mongolian art and traditions', category: 'education' },
      { name: 'Sports & Fitness', description: 'Join workout groups and sports teams', category: 'sports' },
      { name: 'Language Exchange', description: 'Learn Mongolian and other languages together', category: 'education' },
      { name: 'Travel Stories', description: 'Share travel experiences and tips', category: 'social' },
      { name: 'Book Club', description: 'Discuss books and authors', category: 'social' },
    ];

    const groupsToCreate = groupNames.map(g => ({
      name: g.name,
      ...
      privacy: 'public',
      avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${g.name}`,
      cover_image: `https://images.unsplash.com/photo-${Math.floor(Math.random() * 1000)}?w=400&h=300`,
    }));

    const groupsResult = await base44.entities.Group.bulkCreate(groupsToCreate);
    const groups = groupsResult.length > 0 ? groupsResult : groupsToCreate;
    ...
    const postsResult = await base44.entities.Post.bulkCreate(postsToCreate);

    return Response.json({
      success: true,
      groups_created: groups.length || 10,
      posts_created: postsResult.length || 100,
      message: `Seeded ${groups.length || 10} groups and ${postsResult.length || 100} test posts`,
    });
  } catch (error) {
