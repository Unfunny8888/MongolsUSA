/**
 * DiscussionCard — lightweight community post.
 * Tone-aware: lonely, urgent, practical, active, quiet.
 * Keeps UI calm, imperfect, human.
 */
...
// Tone → subtle left-border accent (no loud badges)
const TONE_ACCENT = {
...
function DiscussionCard({ post }) {
  ...
  return (
    <div className={`bg-card border border-border/20 rounded-2xl px-3.5 py-3.5 cursor-pointer active:bg-secondary/20 transition-colors duration-150 ${accentCls}`}>
      ...
      {/* Top reply — only when there are replies */}
      {top_reply && !isQuiet && (
      ...
      <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
        {isQuiet ? (
          <span className="text-primary/60 font-medium flex items-center gap-1">
